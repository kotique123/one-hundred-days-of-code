...
// Benchmark for Foldl and Foldr
func BenchmarkFold(b *testing.B) {
	for _, tc := range foldTestCases {
		b.Run(tc.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				if tc.property == "Foldr" {
					tc.list.Foldr(tc.fn, tc.initial)
				} else {
					tc.list.Foldl(tc.fn, tc.initial)
				}
			}
		})
	}
...
func BenchmarkFilter(b *testing.B) {
	for _, tc := range filterTestCases {
		b.Run(tc.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				tc.list.Filter(tc.fn)
			}
		})
	}
}

// Benchmark for Length
...
func BenchmarkAppend(b *testing.B) {
	for _, tc := range appendTestCases {
		b.Run(tc.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				tc.list.Append(tc.appendThis)
			}
		})
	}
}

// Benchmark for Concat
func BenchmarkConcat(b *testing.B) {
	for _, tc := range concatTestCases {
		b.Run(tc.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				tc.list.Concat(tc.args)
			}
		})
	}
}
